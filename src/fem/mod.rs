...
use conspire::{
    ...
    fem::block::{
        Block, FiniteElementBlock, FiniteElementBlockImprovement, FirstOrderMinimize,
        element::{
            FiniteElementImprovement, FiniteElementMetrics,
        ...
        if !self.get_node_node_connectivity().is_empty() {
            ...
            match *method {
                Smoothing::Energetic => {
                    ...
                    //         .iter()
                    //         .for_each(|nodes| {
                    //             let bar = Hexahedron::scaled_jacobian_gradients(
                    //                 exponent,
                    //                 nodes
                    //                     .iter()
                    ...
                    self.nodal_coordinates = match N {
                        HEX => {
                            let connectivity = self
                                ...
                                .collect();
                            let mut block = Block::<_, Hexahedron, _, _, _, _>::from((
                                NeoHookean {
                                    bulk_modulus: 0.0,
                                    shear_modulus: 1.0,
                                },
                        ...
                        TET => {
                            let connectivity = self
                                ...
                                .collect();
                            let mut block = Block::<_, Tetrahedron, _, _, _, _>::from((
                                NeoHookean {
                                    bulk_modulus: 0.0,
                                    shear_modulus: 1.0,
                                },
...
impl TryFrom<(Tessellation, Size)> for HexahedralFiniteElements {
    ...
    fn try_from((tessellation, size): (Tessellation, Size)) -> Result<Self, Self::Error> {
        ...
        let rounded_coordinates: Vec<_> = nearby_nodes
            .iter()
                    ...
                    surface_node_element_connectivity[closest_node].iter().fold(
                        ...
                        |(closest_triangle, closest_point, minimum_distance_squared), &triangle| {
                            ...
                            let distance_squared =
                                (nearby_node_coordinates - &point).norm_squared();
                            if distance_squared < minimum_distance_squared {
                                (triangle, point, distance_squared)
                            } else {
                                (closest_triangle, closest_point, minimum_distance_squared)
                            }
                        },
                    );
                let cosine = TriangularFiniteElements::normal(
                    &surface_nodal_coordinates,
                    surface_element_node_connectivity[closest_triangle],
                ) * (nearby_node_coordinates - closest_point);
                if cosine > 0.0 || minimum_distance_squared.sqrt() < buffer {
                    Some(nearby_node)
                } else if cosine < 0.0 {
                    None
                } else {
        ...
        // This is empty for non-surface nodes and can be indexed by enumeration.
        let mut special_connectivity = projected_node_nodes;

        finite_elements.node_element_connectivity()?;
        finite_elements.node_node_connectivity()?;
        smart_laplacian_smoothing(
            &mut finite_elements,
            &mut special_connectivity,
            10,
            1e-1,
            Some(1),
        )?;
        //
        // "Smart" = checking if quality of elements involved improves or not.
        //
        // Move nodes back to the surface every so often, or move onto surface each time?
        //
        Ok(finite_elements)
    }
}

fn smart_laplacian_smoothing<'a>(
    finite_elements: &'a mut HexahedralFiniteElements,
    special_connectivity: &'a mut VecConnectivity,
    smoothing_iterations: usize,
    smoothing_scale: Scalar,
    stencil_size: Option<usize>,
) -> Result<(), &'a str> {
    #[cfg(feature = "profile")]
    let time = Instant::now();
    let node_node_connectivity = finite_elements.get_node_node_connectivity();
    if let Some(size) = stencil_size {
        (0..size).for_each(|_| {
            node_node_connectivity
                .iter()
                .zip(special_connectivity.clone())
                .for_each(|(nodes, special)| {
                    if !special.is_empty() {
                        nodes.iter().for_each(|&node| {
                            if special_connectivity[node].is_empty() {
                                special_connectivity[node] = node_node_connectivity[node].clone()
                            }
                        })
                    }
                });
        ...
        special_connectivity
            ...
            .for_each(|(node, nodes)| {
                if nodes.is_empty() {
                    *nodes = node_node_connectivity[node].clone()
                }
            })
    }
    for _ in 0..smoothing_iterations {
        special_connectivity
            ...
            .for_each(|(node, nodes)| {
                let coordinates = finite_elements.get_nodal_coordinates();
                if !nodes.is_empty() {
                    let node_elements = &finite_elements.get_node_element_connectivity()[node];
                    let increment = (nodes
                        .iter()
                        ...
                        - &coordinates[node])
                        * smoothing_scale;
                    let msj_0 = node_elements
                        .iter()
                        .map(|&element| {
                            Hexahedron::minimum_scaled_jacobian(
                                &finite_elements.get_element_node_connectivity()[element]
                                    .iter()
                                    .map(|&edon| coordinates[edon].clone())
                                    .collect(),
                            )
                        })
                        .reduce(f64::min)
                        .unwrap();
                    let msj_p = node_elements
                        .iter()
                        .map(|&element| {
                            Hexahedron::minimum_scaled_jacobian(
                                &finite_elements.get_element_node_connectivity()[element]
                                    ...
                                    .map(|&edon| {
                                        if node == edon {
                                            coordinates[edon].clone() + &increment
                                        } else {
                                            coordinates[edon].clone()
                                        }
                                    })
                                    .collect(),
                            )
                        ...
                        .unwrap();
                    if msj_p >= msj_0 {
                        finite_elements.get_nodal_coordinates_mut()[node] += increment
                    }
                }
            });
    }
    #[cfg(feature = "profile")]
    println!(
        "             \x1b[1;93mSmart nodal smoothing\x1b[0m {:?}",
        time.elapsed()
    );
    Ok(())
}
...
fn closest_surface_node(
    ...
    surface_nodal_coordinates: &Coordinates,
    surface_nodes: &Nodes,
) -> usize {
    surface_nodes
        .iter()
        .fold(
            ...
            |(closest_node, minimum_distance_squared), &surface_node| {
                let distance_squared =
                    (&surface_nodal_coordinates[surface_node] - coordinate).norm_squared();
                if distance_squared < minimum_distance_squared {
...
fn mark_outside_nodes(
    ...
    while index < indices.len() {
        [i, j, k] = indices[index];
        if i > 0 && samples[(i - 1) as usize][j as usize][k as usize] == 2 {
            samples[(i - 1) as usize][j as usize][k as usize] = 0;
            indices.push([i - 1, j, k]);
        }
        if i < lim && samples[(i + 1) as usize][j as usize][k as usize] == 2 {
            samples[(i + 1) as usize][j as usize][k as usize] = 0;
            indices.push([i + 1, j, k]);
        }
        if j > 0 && samples[i as usize][(j - 1) as usize][k as usize] == 2 {
            samples[i as usize][(j - 1) as usize][k as usize] = 0;
            indices.push([i, j - 1, k]);
        }
        if j < lim && samples[i as usize][(j + 1) as usize][k as usize] == 2 {
            samples[i as usize][(j + 1) as usize][k as usize] = 0;
            indices.push([i, j + 1, k]);
        }
        if k > 0 && samples[i as usize][j as usize][(k - 1) as usize] == 2 {
            samples[i as usize][j as usize][(k - 1) as usize] = 0;
            indices.push([i, j, k - 1]);
        }
        if k < lim && samples[i as usize][j as usize][(k + 1) as usize] == 2 {
            samples[i as usize][j as usize][(k + 1) as usize] = 0;
            indices.push([i, j, k + 1]);
        }
        index += 1
    ...
    let removed_and_surface_nodes: Vec<(usize, u8)> = coordinates
        ...
        .filter_map(|(node, coordinate)| {
            ...
            if bar < 2 { Some((node, bar)) } else { None }
        })
        .collect();
    let removed_nodes = removed_and_surface_nodes
        .iter()
        .filter_map(|&(node, bar)| if bar == 0 { Some(node) } else { None })
        .collect();
    let surface_nodes = removed_and_surface_nodes
        .into_iter()
        .filter_map(|(node, bar)| if bar == 1 { Some(node) } else { None })
        .collect();
    #[cfg(feature = "profile")]
    println!(
