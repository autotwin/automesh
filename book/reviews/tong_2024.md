...
* $\mathbf{n}(\mathbf{s})$ is the **inward-pointing** (not outward-pointing as is typically defined) unit normal vector of the surface $\mathcal{S}$ at the closest point $\mathbf{p}_s$.
  ...
  * If $\mathbf{n}(\mathbf{s})$ lies on an edge or or on a vertex, a *pseudo-normal*, which is weighted combination of the normals of the adjoining faces sharing the vertex or edge, is used.
...
Let $\mathbf{q}$ be an augmented vertex of dual hex surface node $\mathbf{p}$.   Let $\mathbf{s}$ be the closet point projection of $\mathbf{q}$ to the surface $\mathcal{S}$.

Since

$$
E_{\rm GF} = \sum_{i=0}^{n_{\rm vert} - 1}
\|
\mathbf{q}_i - \mathbf{s}_i
\|^2
$$
...
$$
\nabla E_{\rm GF} = 2 \sum_{i=0}^{n_{\rm vert} - 1}
\|
\mathbf{q}_i - \mathbf{s}_i
\|
$$

The term acts as a *spring* that pulls the current vertex $\mathbf{q}$ toward its target surface position $\mathbf{s}$.
