...
# 1. Define nodes a, b, c (Initial orthogonal state)
a = np.array([1, 0, 0])
b = np.array([0, 1, 0])
c = np.array([0, 0, 1])

# 2. Set the initial position of node e and a movement direction d
e0 = np.array([0, 0, 0])
d = np.array([1, 0.5, 0.2]) # Arbitrary direction
d = d / np.linalg.norm(d)   # Normalize the direction vector

# 3. Define the range of movement (displacement t)
t_vals = np.linspace(-2, 4, 400)

def compute_metrics(t):
    # Calculate new position of e
    e = e0 + t * d
    
    # Define edge vectors u, v, w
    u = a - e
    v = b - e
    w = c - e

    # Jacobian J = (u x v) . w (Scalar Triple Product)
    j = np.dot(np.cross(u, v), w)

    # Product of edge lengths L = ||u|| * ||v|| * ||w||
    l = np.linalg.norm(u) * np.linalg.norm(v) * np.linalg.norm(w)

    # Scaled Jacobian SJ = J / L
    sj = j / l if l != 0 else 0

    return j, sj

# 4. Generate the data points
...
# Plot the Raw Jacobian (Linear)
plt.plot(t_vals, j_vals, label='Jacobian ($J$)', color='blue', linewidth=2)

# Plot the Scaled Jacobian (Non-linear, dashed magenta)
plt.plot(t_vals, sj_vals, label='Scaled Jacobian ($\hat{J}$)', color='magenta', linestyle='--', linewidth=2)

# Zero threshold (The 'Inversion' cliff)
plt.axhline(0, color='black', linewidth=1.2, linestyle='-')
plt.axvline(0, color='gray', linewidth=0.8, linestyle=':')

# Ideal target line (Green at y = +1)
plt.axhline(1, color='green', linewidth=1.2, linestyle='--', zorder=0, label='Ideal ($y=1$)')

# Lower bound limit (Red at y = -1)
plt.axhline(-1, color='red', linewidth=1.2, linestyle='--', zorder=0, label='Inverted Limit ($y=-1$)')

# Labels and Title
plt.xlabel('Displacement $t$ along arbitrary direction $\mathbf{d}$', fontsize=12)
plt.ylabel('Metric Value', fontsize=12)
plt.title('Jacobian vs. Scaled Jacobian Optimization Landscape', fontsize=14)
...
node $\mathbf{e}$ enters the plane of its neighbors $\mathbf{a}, \mathbf{b}, \mathbf{c}$.
Beyond this point, $J$ becomes negative and the the hex element is inverted.

The **scaled Jacobian** is a *nonlinear* function because the denominator contains the lengths of the edges (square roots of quadratic functions).   The curve is bounded between $[-1, 1]$.  As the node moves very far away, the scaled Jacobian will grow toward zero as the angles between the edges become extremely sharp.  The peak of the curve represents the *ideal* position for node $\mathbf{e}$ relative to its neightbors, and this peak will approach a value of unity for the orthogonal case.

The above plot explores node $\mathbf{e}$ moving along a line parameterized by time $t$ as:

$$\mathbf{e}(t) = \mathbf{e}_0 + t \mathbf{d}$$

where $\mathbf{d}$ is an arbitrary direction and $\mathbf{e}_0$ = $\{0, 0, 0\}$, and the Jacobian functions take the forms
...
1. The Piecewise Energy Function
