...
> **Note on Symmetry:** In a valid emergy formulation, the Hessian must be symmetric.  For the Jacobian $J$, because the skew-symmetry, $\left[\mathbf{\bullet}\right]_{\times}$ is anti-symmetric, $\left[\bullet \right]^T_{\times} = - \left[\bullet\right]_{\times}$.  These interactions provide a *twist* (i.e., a torque) that untangles the element when one nodes moves relative to the others.
