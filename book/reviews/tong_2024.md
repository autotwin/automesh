...
For any hexahedral element that has a negative Jacobian, we seek to maximize the Jacobian energy term $E_{\rm J}$, which will tend to move the Jacobian from negative to positive (untangling).
...
The gradient with respect to $\mathbf{x}$ can be seen by inspection of the three foregoing gradients, with a flip of the sign and use of the chain rule,
...
> **Physical Interpretation:** The gradient $\nabla_x J$ is a vector that describes how the volume (i.e., the Jacobian) changes as the position of node $\mathbf{x}$ changes.  
