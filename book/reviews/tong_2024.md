...
The authors then noted, "During implementation, we observed that this setting [the **vertex clearing rule**] can be sensitive to large elements located in size transition regions, potentially leaving holes in the surface.  To address this issue, we calculate the signed distance function for corner points associated with every hext element.  Each hex had eight signed distance functions $f(\mathbf{x}_i)$, where $i=0, 1, 2 \ldots 7$.  We compute $f_{\min}$ and $f_{\max}$ and remove the hex element if the condition $f_{\min} + 0.1 \times f_{\max} < 0$ is met."
