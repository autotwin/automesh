...
To find the gradient of the Jacobian energy $E_{\rm J}$ with respect to the position of vertex $\mathbf{x}$, we need to differentiate the scalar triple prodcut.  The gradient $\nabla_x E_{\rm SJ}$ is a vector that describes how the volume (i.e., the Jacobian) changes as the position of node $\mathbf{x}$ changes.  
...
Note that $\nabla_x \mathbf{u} = \nabla_x(\mathbf{a} - \mathbf{x}) = - \mathbf{I}$ (and similarly for $\mathbf{v}$ and $\mathbf{w}$), which introduces a negative sign into the derivatives.

Each term in the gradient represents a **vector area** of one of the faces meeting at node $\mathbf{x}$:

* $\mathbf{n}_{ab} = \mathbf{u} \times \mathbf{v}$ is the normal vector to the face formed by $\mathbf{x}, \mathbf{a}, \mathbf{b}$.
* $\mathbf{n}_{bc} = \mathbf{v} \times \mathbf{w}$ is the normal vector to the face formed by $\mathbf{x}, \mathbf{b}, \mathbf{c}$.
* $\mathbf{n}_{ca} = \mathbf{w} \times \mathbf{u}$ is the normal vector to the face formed by $\mathbf{x}, \mathbf{c}, \mathbf{a}$.
...
In terms of the gradient descent update, $x_i \rightarrow x_i - \alpha \nabla E$, we update the position of element node $\mathbf{x}$ as follows
...
The update moves $\mathbf{x}$ in the direction of the sum of the adjacent face normals.  The update effectively "pushes" node $\mathbf{x}$ out of the plane of its neighbors to maximize the volume and resist element inversion.
...
At iteration $k$, we update the position of $\mathbf{x}^{(k)}$ by an amount $-\alpha \nabla_x E$, where $\alpha \in \mathbb{R}^+ \subset (0, 1)$ is the step size (also known as the **learning rate**), which controls how far the vertex $\mathbf{x}$ moves in a single iteration:
